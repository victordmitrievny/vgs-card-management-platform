...
@app.route('/post-clear-data', methods=['POST'])
def handle_client_request():
    if request.method == 'POST':
        ...
        print(f'\nSleeping for a few seconds to wait for NT and AU to provision...\n')
        time.sleep(15)
...
def execute_cmp_workflow():
    time.sleep(4) 
    url = 'https://tntng36c6tl-9d62aca1-ee5c-4d7f-96b5-cd7c34175516.sandbox.verygoodproxy.com/post-clear-data'
    headers = {
        'Content-Type': 'application/json'
    }
    cmp_payload = {
        "amount": "16.50",
        "card_number": "4622943123033772",
        "card_cvc": "123",
        "card_holder": "John Doe",
        "card_exp": "12 / 26"
    }
    requests.post(url, json=cmp_payload, headers=headers)



if __name__ == '__main__':
    # Start Flask server WITHOUT daemon=True (so it stays running)
    server_thread = threading.Thread(target=app.run, kwargs={'debug': False, 'use_reloader': False})
    server_thread.start()

    # Start request function in another thread
    request_thread = threading.Thread(target=execute_cmp_workflow)
    request_thread.start()

    # Keep the main script alive so Flask doesn't die
    server_thread.join()
